...
`;
    ...
    function parseSmartText(text) {
        ...
        if (hasHeader) {
            ...
            // 炸机分析表（7列）：日期 | 机型 | 机架号 | 架次-地块(或W工单号) | 省份 | 初步结论 | 问题定性
            ...
            // 判断第4列语义：
            //   · 工单号 = W 开头（如 W0055555）
            //   · 架次-地块 = 含 -- 的长数字对(178510609957569--3417528) / _FLY_ / FLY 架次 / 9位以上纯数字
            const col3 = firstRowCols[3] || '';
            const col3IsWorkOrder = /^W[\d-]/i.test(col3);
            const col3IsFlightBatch = /_FLY_/i.test(col3) || /FLY[-\s_]\d/i.test(col3) || /--/.test(col3) || /\d{9,}/.test(col3);
